...
class InsurancePolicySource(DataSource):
    ...
    def fetch(self, since: datetime | None = None, limit: int = 0) -> Iterator[RawRecord]:
        ...
        for file_path in self._iter_files():
            ...
            try:
                ...
                log.warning("Failed to parse policy file %s: %s", file_path, e)
    ...
    def _parse_file(self, file_path: Path) -> Iterator[RawRecord]:
        if file_path.suffix == ".json":
            ...
            log.warning("Unsupported file format, skipping: %s", file_path)
    ...
    def _parse_pdf(self, file_path: Path) -> Iterator[RawRecord]:
        ...
        if not text_by_page:
            try:
                ...
                log.warning("pdfplumber failed on %s: %s", file_path, e)
                return

        if not text_by_page:
            log.warning("No extractable text in PDF: %s", file_path)
            return
    ...
    def _parse_html(self, file_path: Path) -> Iterator[RawRecord]:
        ...
        if not text:
            log.warning("No extractable text in HTML: %s", file_path)
            return

#!/usr/bin/env bash
# 本地推送 + 远端 screen 执行 DeepSpeed/Megatron 双卡 SFT+DPO 测试
...
SCREEN_NAME=m_trainer_dual_gpu
...
SSHPASS="${REMOTE_PASS}" sshpass -e ssh -p ${REMOTE_PORT} -o StrictHostKeyChecking=no \
    "${REMOTE_HOST}" "mkdir -p ${REMOTE_ROOT}/logs ${REMOTE_ROOT}/saves/smoke"

log "[3/3] screen 后台执行双卡测试"
SSHPASS="${REMOTE_PASS}" sshpass -e ssh -p ${REMOTE_PORT} -o StrictHostKeyChecking=no \
    "${REMOTE_HOST}" \
    "if screen -ls | grep -q ${SCREEN_NAME}; then screen -X -S ${SCREEN_NAME} quit || true; sleep 2; fi
     cd ${REMOTE_ROOT} && chmod +x deploy/run_m_trainer_dual_gpu_test.sh && \
     screen -dmS ${SCREEN_NAME} bash -c 'bash deploy/run_m_trainer_dual_gpu_test.sh 2>&1 | tee logs/m_trainer_dual_gpu_screen.log; echo DONE > logs/m_trainer_dual_gpu_done.flag'
     sleep 2 && screen -ls"

log "部署完成，日志: ${REMOTE_ROOT}/logs/m_trainer_dual_gpu_screen.log"

#!/usr/bin/env bash
# 本地推送 + 远端 screen 执行 m_infer 全面测试
...
log "[3/3] screen 后台执行 m_infer 测试"
SSHPASS="${REMOTE_PASS}" sshpass -e ssh -p ${REMOTE_PORT} -o StrictHostKeyChecking=no \
    "${REMOTE_HOST}" \
    "if screen -ls | grep -q ${SCREEN_NAME}; then screen -X -S ${SCREEN_NAME} quit || true; sleep 2; fi
     cd ${REMOTE_ROOT} && chmod +x deploy/run_m_infer_test.sh && \
     screen -dmS ${SCREEN_NAME} bash -c 'bash deploy/run_m_infer_test.sh 2>&1 | tee logs/m_infer_test_screen.log; echo DONE > logs/m_infer_test_done.flag'
     sleep 2 && screen -ls"

log "部署完成，日志: ${REMOTE_ROOT}/logs/m_infer_test_screen.log"
